...
export function MatrimonialEngine({ activeCase }: Props) {
  ...
  // ── Helper: format children from extraction ───────────────────────────────
  ...
  function IntakePanel() {
    ...
    // Pre-populate from intelligence if no saved intake data
  ...
  function PetitionPanel() {
    ...
    // Pre-select the strongest identified fact
    const bestFact = intel?.dissolution_facts?.find(f => f.strength === 'STRONG' || f.strength === 'MODERATE');
    const [ground,       setGround]       = useState(saved.petGround       || bestFact?.fact || '');
    ...
    return (
      <div>
        <div style={cardS}>
          ...
          {/* Welfare concerns from intelligence */}
