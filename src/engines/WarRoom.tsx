    ...
    <div style={{ background: '#06060f', border: '1px solid #101020', borderRadius: 6, padding: '18px 20px', marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, flexWrap: 'wrap', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 11, color: T.gold, opacity: .5 }}>{icon}</span>
          <span style={{ fontSize: 13, color: T.goldL, fontFamily: "'Cormorant Garamond', serif", fontWeight: 600, letterSpacing: '.04em' }}>{title}</span>
        </div>
        {onGenerate && (
          <button
            onClick={onGenerate}
...
function EmptyPanel({ msg, actions }: { msg: string; actions?: React.ReactNode }) {
  return (
    <div style={{ textAlign: 'center', padding: '20px 0' }}>
      <p style={{ fontSize: 12, color: '#303040', fontFamily: 'Inter, sans-serif', fontStyle: 'italic', marginBottom: actions ? 10 : 0 }}>{msg}</p>
...
function NavBtn({ label, tab, setDashTab }: { label: string; tab: DashTabId; setDashTab: (t: DashTabId) => void }) {
  return (
    <button
      ...
      style={{ background: 'transparent', border: '1px solid #1e1e30', color: '#505070', borderRadius: 4, padding: '4px 12px', fontSize: 10, fontFamily: 'Inter, sans-serif', cursor: 'pointer' }}
    >
      {label} ›
    </button>
  );
...
export function WarRoom({ activeCase }: Props) {
  ...
  async function aiGenerate(
    ...
    setErr(key, '');
    try {
      const text = await callClaude({ system, userMsg, maxTokens: 1800 });
      setter(text);
      writeLS(lsKey, text);
    } catch (e) {
  ...
  function renderTheory() {
    return (
      ...
      >
        {errors['theory'] && (
          <p style={{ fontSize: 12, color: '#c05050', fontFamily: 'Inter, sans-serif', marginBottom: 10 }}>{errors['theory']}</p>
        )}
        {caseTheory ? (
          <Md text={caseTheory} />
        ) : (
          intel.legal_issues ? (
            <div>
              <p style={{ fontSize: 11, color: T.dim, fontFamily: 'Inter, sans-serif', marginBottom: 10, fontStyle: 'italic' }}>
                Derived from Intelligence Package — generate for full AI analysis:
              </p>
              {issueList.map((issue, i) => (
                <div key={i} style={{ background: '#0a0a18', border: '1px solid #151525', borderRadius: 4, padding: '8px 12px', marginBottom: 6, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  <span style={{ fontSize: 9, color: '#5050a0', fontFamily: 'Inter, sans-serif', letterSpacing: '.1em', textTransform: 'uppercase', flexShrink: 0, paddingTop: 1 }}>Issue {i + 1}</span>
                  <span style={{ fontSize: 12, color: '#b0acaa', fontFamily: 'Inter, sans-serif', lineHeight: 1.6 }}>{issue}</span>
                </div>
              ))}
            </div>
          ) : (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
          {([
            ...
            <div key={l} style={{ background: '#0a0a18', border: '1px solid #141424', borderRadius: 4, padding: '8px 12px' }}>
              <div style={{ fontSize: 9, color: '#404060', fontFamily: 'Inter, sans-serif', letterSpacing: '.12em', textTransform: 'uppercase', marginBottom: 3 }}>{l}</div>
              <div style={{ fontSize: 13, color: '#d0ccc0', fontFamily: "'Cormorant Garamond', serif" }}>{v}</div>
            </div>
          ))}
        </div>
        <div style={{ background: '#090916', border: '1px solid #1e1e3a', borderRadius: 5, padding: '12px 16px', marginBottom: 12 }}>
          <div style={{ fontSize: 9, color: '#505080', fontFamily: 'Inter, sans-serif', letterSpacing: '.14em', textTransform: 'uppercase', marginBottom: 6 }}>
            Current Posture (set in Command Console)
          </div>
          <div style={{ fontSize: 16, color: '#a0a0e0', fontFamily: "'Cormorant Garamond', serif", fontWeight: 600, marginBottom: 6 }}>{currentPosture}</div>
          <div style={{ fontSize: 12, color: '#808090', fontFamily: 'Inter, sans-serif', lineHeight: 1.65 }}>
            {POSTURE_ADVICE[currentPosture] || 'No posture set.'}
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 10, color: T.dim, fontFamily: 'Inter, sans-serif', alignSelf: 'center' }}>Switch posture in →</span>
        ...
        {allWitnesses.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 10 }}>
            {allWitnesses.map((w, i) => {
              ...
              return (
                <div key={i} style={{ background: '#0a0a18', border: `1px solid ${isOwn ? '#1a2a1a' : '#2a1a1a'}`, borderRadius: 5, padding: '10px 12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span style={{ fontSize: 12, color: '#c8c4b8', fontFamily: "'Cormorant Garamond', serif", fontWeight: 600 }}>
                      {w.name || `Witness ${i + 1}`}
                    </span>
                    <span style={{ fontSize: 8, color: isOwn ? '#50a060' : '#c05050', fontFamily: 'Inter, sans-serif', letterSpacing: '.1em', textTransform: 'uppercase' }}>
                      {isOwn ? 'OURS' : 'OPP'}
                    </span>
                  </div>
                  <div style={{ fontSize: 10, color: '#505060', fontFamily: 'Inter, sans-serif', marginBottom: 4 }}>{w.role || w.type || 'Witness'}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: 10, color: isReady ? '#50c070' : '#d09030', fontFamily: 'Inter, sans-serif' }}>{status}</span>
                  </div>
                  {vuln && (
                    <div style={{ fontSize: 10, color: '#906050', fontFamily: 'Inter, sans-serif', lineHeight: 1.5, borderTop: '1px solid #151520', paddingTop: 6, marginTop: 4 }}>
                      {vuln.slice(0, 80)}{vuln.length > 80 ? '…' : ''}
                    </div>
                  )}
                  <button
                    onClick={() => setDashTab('crossexam')}
                    style={{ marginTop: 8, background: 'transparent', border: '1px solid #1e1e30', color: '#404060', borderRadius: 3, padding: '3px 9px', fontSize: 9, fontFamily: 'Inter, sans-serif', cursor: 'pointer', width: '100%' }}
                  >
                    CX Prep ›
                  </button>
                </div>
      ...
      <PanelWrap title="Evidence Coverage Board" icon="📁">
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <span style={{ fontSize: 11, color: T.dim, fontFamily: 'Inter, sans-serif' }}>Evidence Vault:</span>
            <span style={{ fontSize: 13, color: T.goldL, fontFamily: "'Cormorant Garamond', serif" }}>
              {evMeta.length} document{evMeta.length !== 1 ? 's' : ''}
            </span>
            {evMeta.length > 0 && (
              <button onClick={() => setDashTab('evidence')} style={{ marginLeft: 'auto', background: 'transparent', border: '1px solid #1e1e30', color: '#404060', borderRadius: 3, padding: '3px 9px', fontSize: 9, fontFamily: 'Inter, sans-serif', cursor: 'pointer' }}>
                Open Vault ›
              </button>
            )}
          </div>
          {evCategories.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
              {evCategories.map((cat, i) => (
                <span key={i} style={{ fontSize: 10, color: '#50a070', fontFamily: 'Inter, sans-serif', border: '1px solid #1a3a2a', background: '#050f0a', borderRadius: 3, padding: '3px 9px' }}>{cat}</span>
              ))}
            </div>
          )}
        </div>
        ...
        {issueList.length > 0 ? (
          <div>
            <div style={{ fontSize: 10, color: '#404060', fontFamily: 'Inter, sans-serif', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 8 }}>Issue Coverage Matrix</div>
            {issueList.map((issue, i) => {
              ...
              return (
                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '7px 10px', background: '#08080f', border: '1px solid #0e0e1e', borderRadius: 4, marginBottom: 5 }}>
                  ...
                  <span style={{ fontSize: 11, color: '#b0acaa', fontFamily: 'Inter, sans-serif', lineHeight: 1.5, flex: 1 }}>{issue}</span>
                  <span style={{ fontSize: 9, color: covered ? '#50a060' : '#a04040', fontFamily: 'Inter, sans-serif', flexShrink: 0, letterSpacing: '.08em' }}>
                    {covered ? 'COVERED' : 'GAP'}
                  </span>
                </div>
              );
            ...
            {intel.extraction?.gaps_identified && intel.extraction.gaps_identified.length > 0 && (
              <div style={{ marginTop: 12, background: '#120808', border: '1px solid #2a1010', borderRadius: 4, padding: '10px 12px' }}>
                <div style={{ fontSize: 9, color: '#803030', fontFamily: 'Inter, sans-serif', letterSpacing: '.12em', textTransform: 'uppercase', marginBottom: 5 }}>Intelligence Engine — Gaps Identified</div>
                <ul style={{ margin: 0, padding: '0 0 0 16px' }}>
                  {intel.extraction.gaps_identified.map((g, i) => (
                    <li key={i} style={{ fontSize: 11, color: '#a06060', fontFamily: 'Inter, sans-serif', lineHeight: 1.65, marginBottom: 3 }}>{g}</li>
                  ))}
                </ul>
              </div>
            )}
    ...
    return (
      <PanelWrap title="Contradiction Tracker" icon="⚔">
        {contradictions.length === 0 ? (
          ...
          <div>
            <div style={{ fontSize: 10, color: T.dim, fontFamily: 'Inter, sans-serif', marginBottom: 10 }}>
            ...
            {contradictions.map((c, i) => (
              <div key={i} style={{ background: '#0f0808', border: '1px solid #2a1515', borderRadius: 5, padding: '10px 14px', marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  <span style={{ fontSize: 9, color: '#903030', fontFamily: 'Inter, sans-serif', letterSpacing: '.12em', textTransform: 'uppercase', border: '1px solid #2a1010', padding: '2px 7px', borderRadius: 2 }}>
                    Contradiction {i + 1}
                  </span>
                  {c.witness && <span style={{ fontSize: 10, color: '#c08060', fontFamily: 'Inter, sans-serif' }}>— {c.witness}</span>}
                  {c.severity && (
                    <span style={{ marginLeft: 'auto', fontSize: 9, color: c.severity === 'High' ? '#e05050' : c.severity === 'Medium' ? '#d09030' : '#808080', fontFamily: 'Inter, sans-serif', textTransform: 'uppercase', letterSpacing: '.06em' }}>
                      {c.severity}
                    </span>
                  )}
                </div>
                {c.statement1   && <div style={{ fontSize: 11, color: '#906060', fontFamily: 'Inter, sans-serif', lineHeight: 1.5, marginBottom: 4 }}>Statement: {c.statement1}</div>}
                {c.statement2   && <div style={{ fontSize: 11, color: '#609060', fontFamily: 'Inter, sans-serif', lineHeight: 1.5, marginBottom: 4 }}>Contradicted by: {c.statement2}</div>}
                {c.exploitation && <div style={{ fontSize: 11, color: '#808060', fontFamily: 'Inter, sans-serif', lineHeight: 1.5, fontStyle: 'italic' }}>Exploit: {c.exploitation}</div>}
              </div>
            ))}
          </div>
        )}
      ...
      <PanelWrap title="Risk Alerts" icon="■">
        {overdueDl.length > 0 && (
          <div style={{ background: '#130505', border: '1px solid #3a1010', borderRadius: 5, padding: '10px 14px', marginBottom: 12 }}>
            <div style={{ fontSize: 9, color: '#e05050', fontFamily: 'Inter, sans-serif', letterSpacing: '.14em', textTransform: 'uppercase', marginBottom: 8 }}>
              ⚠ OVERDUE DEADLINES ({overdueDl.length})
            </div>
            {overdueDl.slice(0, 3).map((d, i) => (
              <div key={i} style={{ fontSize: 11, color: '#d07070', fontFamily: 'Inter, sans-serif', lineHeight: 1.6, marginBottom: 3 }}>
                • {d.label} — {new Date(d.date).toLocaleDateString('en-GB')}
              </div>
            ))}
            <button onClick={() => setDashTab('docket')} style={{ marginTop: 8, background: 'transparent', border: '1px solid #2a1010', color: '#804040', borderRadius: 3, padding: '3px 9px', fontSize: 9, fontFamily: 'Inter, sans-serif', cursor: 'pointer' }}>
              Manage Deadlines ›
            </button>
          </div>
        )}
        ...
        {urgentDl.length > 0 && (
          <div style={{ background: '#120a03', border: '1px solid #3a2010', borderRadius: 5, padding: '10px 14px', marginBottom: 12 }}>
            <div style={{ fontSize: 9, color: '#d09030', fontFamily: 'Inter, sans-serif', letterSpacing: '.14em', textTransform: 'uppercase', marginBottom: 8 }}>
              ⏱ URGENT (within 7 days) ({urgentDl.length})
            </div>
            {urgentDl.slice(0, 3).map((d, i) => (
              <div key={i} style={{ fontSize: 11, color: '#c0a060', fontFamily: 'Inter, sans-serif', lineHeight: 1.6, marginBottom: 3 }}>
                • {d.label} — {new Date(d.date).toLocaleDateString('en-GB')}
              </div>
            ))}
          </div>
        ...
        {topRisks.length > 0 ? (
          <div>
            <div style={{ fontSize: 10, color: '#404060', fontFamily: 'Inter, sans-serif', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 8 }}>
              Top Risk Dimensions (from Risk Analytics)
            </div>
            {topRisks.map(([key, score], i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 12px', background: '#080810', border: '1px solid #101020', borderRadius: 4, marginBottom: 6 }}>
                <div style={{ width: 32, height: 32, borderRadius: '50%', background: riskColor(score), display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <span style={{ fontSize: 11, color: '#fff', fontWeight: 700, fontFamily: 'Inter, sans-serif' }}>{score}</span>
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, color: '#c0bcb4', fontFamily: 'Inter, sans-serif', textTransform: 'capitalize' }}>{key.replace(/_/g, ' ')}</div>
                  <div style={{ fontSize: 9, color: riskColor(score), fontFamily: 'Inter, sans-serif', letterSpacing: '.1em', textTransform: 'uppercase' }}>{riskLabel(score)}</div>
                </div>
                <button onClick={() => setDashTab('risk')} style={{ background: 'transparent', border: '1px solid #1e1e30', color: '#404060', borderRadius: 3, padding: '3px 8px', fontSize: 9, fontFamily: 'Inter, sans-serif', cursor: 'pointer' }}>→</button>
              </div>
            ))}
          </div>
        ) : !allClear ? null : null}
        ...
        {allClear && (
          <div style={{ textAlign: 'center', padding: '10px 0' }}>
            <span style={{ fontSize: 12, color: '#30c060', fontFamily: 'Inter, sans-serif' }}>✓ No immediate risk alerts</span>
          </div>
        )}

        {topRisks.length === 0 && !allClear && (
          <p style={{ fontSize: 12, color: '#303040', fontFamily: 'Inter, sans-serif', fontStyle: 'italic' }}>No risk scores yet. Run Risk Analytics to populate this panel.</p>
        )}
      </PanelWrap>
    );
  }
  ...
  function renderAppellate() {
    return (
      ...
      >
        {errors['appvuln'] && <p style={{ fontSize: 12, color: '#c05050', fontFamily: 'Inter, sans-serif', marginBottom: 10 }}>{errors['appvuln']}</p>}
        {appellateVuln ? (
        ...
        <button onClick={() => setDashTab('appeal')} style={{ marginTop: 10, background: 'transparent', border: '1px solid #1e1e30', color: '#404060', borderRadius: 4, padding: '4px 12px', fontSize: 10, fontFamily: 'Inter, sans-serif', cursor: 'pointer' }}>
          Appeal Engine ›
        </button>
      </PanelWrap>
    );
  ...
  function renderOpponent() {
    return (
      ...
      >
        {errors['opp'] && <p style={{ fontSize: 12, color: '#c05050', fontFamily: 'Inter, sans-serif', marginBottom: 10 }}>{errors['opp']}</p>}
        {oppStrategy ? (
          <Md text={oppStrategy} />
        ) : (
      ...
      <PanelWrap title="Hearing Readiness Board" icon="⚖">
        {nextHearing ? (
          <div style={{ background: '#080814', border: '1px solid #1a1a2a', borderRadius: 5, padding: '10px 14px', marginBottom: 14 }}>
            <div style={{ fontSize: 9, color: '#505080', fontFamily: 'Inter, sans-serif', letterSpacing: '.12em', textTransform: 'uppercase', marginBottom: 4 }}>Next Hearing</div>
            <div style={{ fontSize: 14, color: '#a0a0d0', fontFamily: "'Cormorant Garamond', serif" }}>{nextHearing.docTitle || 'Hearing'}</div>
            <div style={{ fontSize: 11, color: '#606080', fontFamily: 'Inter, sans-serif', marginTop: 2 }}>
          ...
          <div style={{ background: '#080810', border: '1px solid #141420', borderRadius: 5, padding: '8px 14px', marginBottom: 14 }}>
            <span style={{ fontSize: 11, color: '#303040', fontFamily: 'Inter, sans-serif', fontStyle: 'italic' }}>No upcoming hearings in docket</span>
          </div>
        )}

        <div style={{ marginBottom: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <span style={{ fontSize: 10, color: T.dim, fontFamily: 'Inter, sans-serif', letterSpacing: '.1em', textTransform: 'uppercase' }}>Readiness</span>
            <span style={{ fontSize: 12, color: doneCount === checks.length ? '#50c070' : doneCount > 4 ? '#d09030' : '#e05050', fontFamily: "'Cormorant Garamond', serif" }}>
          ...
          {checks.map((ch, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '5px 0', borderBottom: '1px solid #0e0e18' }}>
              ...
              <span style={{ fontSize: 11, color: ch.done ? '#708060' : '#505060', fontFamily: 'Inter, sans-serif', lineHeight: 1.4 }}>{ch.label}</span>
            </div>
          ))}
        </div>
        <button onClick={() => setDashTab('briefme')} style={{ marginTop: 8, background: 'transparent', border: '1px solid #1e1e30', color: '#505070', borderRadius: 4, padding: '5px 14px', fontSize: 10, fontFamily: 'Inter, sans-serif', cursor: 'pointer', width: '100%' }}>
  ...
  function renderJudicial() {
    ...
    return (
      <PanelWrap title="Judicial Notes" icon="§">
        <p style={{ fontSize: 11, color: T.dim, fontFamily: 'Inter, sans-serif', marginBottom: 12, fontStyle: 'italic' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
          <textarea
            ...
            rows={3}
            style={{ flex: 1, background: '#070710', border: '1px solid #141424', borderRadius: 4, color: '#c0bcb4', fontSize: 11, fontFamily: 'Inter, sans-serif', padding: '8px 10px', resize: 'vertical', outline: 'none', lineHeight: 1.65 }}
          />
          <button onClick={addNote} style={{ background: 'transparent', border: '1px solid #2a2a4a', color: '#6060a0', borderRadius: 4, padding: '8px 14px', fontSize: 11, fontFamily: 'Inter, sans-serif', cursor: 'pointer', alignSelf: 'flex-start', flexShrink: 0 }}>
            Add
          </button>
        </div>
        {judicialLog.length === 0 ? (
          <p style={{ fontSize: 12, color: '#252535', fontFamily: 'Inter, sans-serif', fontStyle: 'italic' }}>No judicial notes yet.</p>
        ) : (
          <div>
            {[...judicialLog].reverse().map(n => (
              <div key={n.id} style={{ background: '#080810', border: '1px solid #101020', borderRadius: 4, padding: '10px 12px', marginBottom: 8, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, color: '#b0acaa', fontFamily: 'Inter, sans-serif', lineHeight: 1.65, whiteSpace: 'pre-wrap' }}>{n.text}</div>
    ...
    <div style={{ padding: '0 0 32px 0', animation: 'fadeUp .3s ease' }}>
      ...
      <div style={{ marginBottom: 22 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
          ...
          <h2 style={{ margin: 0, fontSize: 22, color: T.goldL, fontFamily: "'Cormorant Garamond', serif", fontWeight: 400, letterSpacing: '.04em' }}>War Room</h2>
          <span style={{ fontSize: 9, color: '#303038', fontFamily: 'Inter, sans-serif', letterSpacing: '.18em', textTransform: 'uppercase', border: '1px solid #1a1a28', padding: '2px 8px', borderRadius: 2 }}>
            Strategic Cockpit
          </span>
        </div>
        <p style={{ margin: '0 0 0 24px', fontSize: 12, color: T.dim, fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: 0, flexWrap: 'wrap', marginBottom: 20, border: '1px solid #101018', borderRadius: 5, overflow: 'hidden' }}>
        {PANELS.map(p => (
          <button
            ...
            style={{
              background:    activePanel === p.id ? '#0e0e1e' : 'transparent',
              border:        'none',
