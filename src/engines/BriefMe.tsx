...
const BM_SECTIONS: SectionConfig[] = [
  { id: 'today_matter',         icon: '📋', label: "Today's Matter",              col: '#c4a030', bg: '#100d02', bdr: '#2a2208' },
    ...
    <div style={{ animation: 'fadeUp .3s ease' }} ref={topRef}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 22, paddingBottom: 20, borderBottom: `1px solid ${T.bdr}` }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 9, color: acc, fontFamily: 'Inter, sans-serif', letterSpacing: '.2em', textTransform: 'uppercase', fontWeight: 600 }}>Brief Me · Step 9</span>
            {briefing && (
      ...
      {!hasEnoughContext && (
        <div style={{ background: '#0e0a04', border: '1px solid #3a2808', borderRadius: 8, padding: '14px 18px', marginBottom: 18, display: 'flex', alignItems: 'flex-start', gap: 12 }}>
          ...
          <div>
            <p style={{ fontSize: 13, color: '#c09040', fontFamily: "'Cormorant Garamond', serif", fontWeight: 600, marginBottom: 3 }}>Thin case file detected</p>
      ...
      <div style={{ background: '#0d0d18', border: `1px solid ${T.bdr}`, borderRadius: 10, padding: '22px 24px', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginBottom: 18 }}>
          <button
            ...
            style={{
              ...
              border: `1px solid ${useDrive ? acc + '50' : '#1e1e2e'}`,
              color: useDrive ? acc : T.mute,
              borderRadius: 4, padding: '5px 13px', fontSize: 10,
              fontFamily: 'Inter, sans-serif', cursor: 'pointer',
              letterSpacing: '.04em', display: 'flex', alignItems: 'center', gap: 5,
            }}
          >
            <span style={{ width: 5, height: 5, borderRadius: '50%', background: useDrive ? acc : '#2a2a3e', display: 'inline-block', flexShrink: 0 }} />
            Google Drive RAG
          </button>
          <span style={{ fontSize: 10, color: '#2a2a3e', fontFamily: 'Inter, sans-serif' }}>
        ...
        <button
          ...
          style={{
            width: '100%',
            background: loading ? '#0e0e18' : `linear-gradient(135deg,#c4a030,#a07820)`,
        ...
        >
          {loading ? (
            <>
              <span style={{ width: 14, height: 14, border: '2px solid #1a1400', borderTop: `2px solid ${acc}`, borderRadius: '50%', display: 'inline-block', animation: 'spin .8s linear infinite' }} />
      ...
      {briefing && (
        <div style={{ animation: 'fadeUp .35s ease' }}>
          ...
          {/* Briefing header */}
          <div style={{ background: '#0a0a14', border: `1px solid ${acc}33`, borderRadius: 10, padding: '18px 22px', marginBottom: 18, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 14, flexWrap: 'wrap' }}>
            <div>
              <p style={{ fontSize: 9, color: acc, fontFamily: 'Inter, sans-serif', letterSpacing: '.2em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 5 }}>
                Pre-Hearing Briefing · Ready
              </p>
              <h3 style={{ fontSize: 20, color: T.goldL, fontFamily: "'Cormorant Garamond', serif", fontWeight: 300, marginBottom: 4 }}>
                {activeCase.caseName}
              </h3>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                {briefing.hearingDate && (
                  <span style={{ fontSize: 10, color: '#b07030', fontFamily: 'Inter, sans-serif', border: '1px solid #3a2208', padding: '2px 9px', borderRadius: 2 }}>
                    ⏱ {new Date(`${briefing.hearingDate}T12:00:00`).toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
                  </span>
                )}
                {briefing.courtName && (
                  <span style={{ fontSize: 10, color: T.dim, fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic' }}>
                    {briefing.courtName}
                  </span>
                )}
            ...
            <div style={{ display: 'flex', gap: 8, flexShrink: 0, flexWrap: 'wrap' }}>
              <button
                onClick={copyAll}
                style={{ background: allCopied ? '#071808' : 'transparent', border: `1px solid ${allCopied ? '#1a4018' : '#1e1e2e'}`, color: allCopied ? '#40a858' : T.mute, borderRadius: 4, padding: '6px 14px', fontSize: 10, fontFamily: 'Inter, sans-serif', cursor: 'pointer', letterSpacing: '.04em' }}
              >
                {allCopied ? '✓ Copied' : 'Copy All'}
              </button>
              <button
                onClick={clearBriefing}
                style={{ background: 'transparent', border: '1px solid #2a1a1a', color: '#6a3030', borderRadius: 4, padding: '6px 14px', fontSize: 10, fontFamily: 'Inter, sans-serif', cursor: 'pointer' }}
              >
                Clear
              </button>
            </div>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 9, color: sec.col, fontFamily: 'Inter, sans-serif', letterSpacing: '.16em', textTransform: 'uppercase', fontWeight: 600 }}>
                      {sec.label}
                    </span>
                  </div>
                  <button
                    onClick={() => copySection(sec.id, text)}
                    style={{ background: copied === sec.id ? '#071808' : 'transparent', border: `1px solid ${copied === sec.id ? '#1a4018' : '#1e1e2e'}`, color: copied === sec.id ? '#40a858' : T.mute, borderRadius: 3, padding: '3px 11px', fontSize: 9, fontFamily: 'Inter, sans-serif', cursor: 'pointer', letterSpacing: '.06em' }}
                  >
                    {copied === sec.id ? '✓ Copied' : 'Copy'}
                  </button>
                </div>
                <div style={{ fontSize: 14, color: `${sec.col}dd`, fontFamily: "'Cormorant Garamond', serif", lineHeight: 1.85, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                  {text}
                </div>
              </div>
            );
          ...
          <p style={{ fontSize: 10, color: '#1e1e2e', fontFamily: 'Inter, sans-serif', lineHeight: 1.7, textAlign: 'center', marginTop: 8 }}>
      ...
      {!briefing && !loading && (
        <div style={{ textAlign: 'center', padding: '60px 24px', background: '#07070e', border: '1px dashed #141428', borderRadius: 10 }}>
          ...
          <p style={{ fontSize: 12, color: T.mute, fontFamily: 'Inter, sans-serif', lineHeight: 1.85, maxWidth: 440, margin: '0 auto' }}>
