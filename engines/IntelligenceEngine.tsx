...
const RISK_SEV_C: Record<string, { bg: string; bdr: string; col: string }> = {
  HIGH:   { bg: '#1a0808', bdr: '#4a1818', col: '#c05050' },
...
const PRIORITY_C: Record<string, { bg: string; bdr: string; col: string }> = {
  CRITICAL: { bg: '#1a0808', bdr: '#4a1818', col: '#c05050' },
...
Defendants: ${activeCase.defendants.map(d => d.name).filter(Boolean).join(', ') || 'Not named'}`;
  ...
  async function runExtraction() {
    ...
    const prompt = `${caseCtx}\n\nRAW FACTS / CLIENT NARRATION:\n${rawFacts}\n\nExtract structured trial intelligence. Output ONLY valid JSON — no markdown fences, no preamble:\n{\n  "timeline": [{"date":"...","event":"...","significance":"..."}],\n  "established_facts": ["..."],\n  "disputed_areas": ["..."],\n  "legal_issues": ["..."],\n  "evidence_mentioned": ["..."],\n  "gaps_identified": ["..."],\n  "initial_risks": [{"risk":"...","severity":"HIGH|MEDIUM|LOW"}]\n}`;
    try {
      const raw = await callClaude({
        system: `You are a trial intelligence extraction engine for Nigerian litigation. Extract structured intelligence from raw case facts. Role-aware: the lawyer acts for the ${role}. Output ONLY valid JSON with no markdown.`,
        userMsg: prompt,
        maxTokens: 2000,
      });
      const cleaned = raw.replace(/```json|```/g, '').trim();
      const ext: ExtractionResult = JSON.parse(cleaned);
      setExtraction(ext);
  ...
  async function generateFollowUp() {
    setLoading(true); setError('');
    const prompt = `${caseCtx}\n\nEXTRACTED INTELLIGENCE:\n${JSON.stringify(extraction, null, 2)}\n\nGenerate 6 targeted follow-up questions addressing the most critical gaps. Output ONLY valid JSON:\n{"questions":[{"id":"q1","question":"...","purpose":"..."}]}`;
    try {
      const raw = await callClaude({
        system: `You are a trial intelligence engine for Nigerian litigation. Generate precise gap-filling follow-up questions. Role: ${role}. Output ONLY valid JSON.`,
        userMsg: prompt,
        maxTokens: 1500,
      });
      const cleaned = raw.replace(/```json|```/g, '').trim();
      const qs: TIEData['followUpQs'] = JSON.parse(cleaned).questions || [];
  ...
  async function buildEvidenceMatrix() {
    ...
    const qaText = followUpQs
      ...
      .join('\n\n');
    const prompt = `${caseCtx}\n\nEXTRACTED INTELLIGENCE:\n${JSON.stringify(extraction, null, 2)}\n\nFOLLOW-UP ANSWERS:\n${qaText}\n\nBuild the evidence matrix. Output ONLY valid JSON:\n{"evidence_map":[{"issue":"...","evidence_needed":["..."],"evidence_available":["..."],"evidence_missing":["..."],"priority":"CRITICAL|HIGH|MEDIUM|LOW","notes":"..."}]}`;
    try {
      const raw = await callClaude({
        system: `You are a trial evidence strategist for Nigerian litigation. Map required evidence to facts and legal issues. Role of client: ${role}. Output ONLY valid JSON.`,
        userMsg: prompt,
        maxTokens: 2500,
      });
      const cleaned = raw.replace(/```json|```/g, '').trim();
      const em: EvidenceMapItem[] = JSON.parse(cleaned).evidence_map || [];
      setEvidenceM(em);
            ...
            <React.Fragment key={s.id}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, flexShrink: 0, minWidth: 68 }}>
                <div style={{
                  ...
                  border: `2px solid ${done ? '#2a6a40' : active ? ACC : '#1a1a28'}`,
                  color: done ? '#40b068' : active ? ACC : '#2a2a3e',
                  transition: 'all .3s', flexShrink: 0,
                }}>
                  {done ? '✓' : s.id}
                </div>
                <span style={{
                  fontSize: 8, color: done ? '#40b068' : active ? ACC : '#2a2a3e',
                  fontFamily: 'Inter, sans-serif', letterSpacing: '.07em',
    ...
    return (
      <div style={{ textAlign: 'center', padding: '54px 24px' }}>
        <div style={{
          ...
          borderTop: `2px solid ${ACC}`, borderRadius: '50%',
          margin: '0 auto 18px', animation: 'spin .9s linear infinite',
        }} />
        <p style={{ fontSize: 19, color: T.goldL, fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic' }}>
          {label}
        </p>
      </div>
    );
    ...
    return (
      <div style={{ animation: 'fadeUp .3s ease' }}>
        <div style={{ marginBottom: 20 }}>
          <p style={{ fontSize: 10, color: ACC, fontFamily: 'Inter, sans-serif', letterSpacing: '.16em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 4 }}>
        ...
        <div style={{ background: '#0d0d18', border: '1px solid #181828', borderRadius: 10, padding: '20px 22px', marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, paddingBottom: 12, borderBottom: '1px solid #131320', flexWrap: 'wrap' }}>
            <RoleBadge role={role} />
            <span style={{ fontSize: 11, color: T.mute, fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic' }}>
              {activeCase.caseName}
            </span>
            {activeCase.court && (
              <span style={{ fontSize: 10, color: '#2a2a3e', fontFamily: 'Inter, sans-serif' }}>· {activeCase.court}</span>
            )}
          </div>
          <label style={lbS}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
            <span style={{ fontSize: 10, color: rawFacts.length < 50 ? '#804040' : T.mute, fontFamily: 'Inter, sans-serif' }}>
              {rawFacts.length} characters{rawFacts.length < 50 ? ' · minimum 50' : ''}
            </span>
            <span style={{ fontSize: 10, color: T.mute, fontFamily: 'Inter, sans-serif', letterSpacing: '.04em' }}>
        ...
        <button
          ...
          style={{
            background: loading || rawFacts.trim().length < 50
              ? '#101018'
              : 'linear-gradient(135deg,#c4a030,#a07820)',
    ...
    return (
      <div style={{ animation: 'fadeUp .3s ease' }}>
        <div style={{ marginBottom: 18 }}>
          <p style={{ fontSize: 10, color: '#40a868', fontFamily: 'Inter, sans-serif', letterSpacing: '.16em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 4 }}>
        ...
        {extraction.timeline?.length > 0 && (
          <div style={{ background: '#0d0d18', border: '1px solid #181828', borderRadius: 8, padding: '18px 20px', marginBottom: 12 }}>
            <p style={{ fontSize: 9, color: ACC, fontFamily: 'Inter, sans-serif', letterSpacing: '.14em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 14 }}>
              Case Timeline
            </p>
            {extraction.timeline.map((t, i) => (
              <div key={i} style={{ display: 'flex', gap: 14, marginBottom: 10, paddingBottom: 10, borderBottom: i < extraction.timeline.length - 1 ? '1px solid #131320' : 'none' }}>
                <div style={{ flexShrink: 0, width: 8, height: 8, borderRadius: '50%', background: ACC, marginTop: 6 }} />
                <div style={{ flex: 1 }}>
                  <span style={{ fontSize: 11, color: ACC, fontFamily: 'Inter, sans-serif', fontWeight: 600, display: 'block', marginBottom: 2 }}>{t.date}</span>
                  <p style={{ fontSize: 14, color: T.text, fontFamily: "'Cormorant Garamond', serif", lineHeight: 1.6, marginBottom: t.significance ? 3 : 0 }}>{t.event}</p>
                  {t.significance && (
                    <p style={{ fontSize: 11, color: T.mute, fontFamily: 'Inter, sans-serif', lineHeight: 1.5, fontStyle: 'italic' }}>{t.significance}</p>
                  )}
                </div>
              </div>
            ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
          {extraction.established_facts?.length > 0 && (
            <div style={{ background: '#071810', border: '1px solid #1a4028', borderRadius: 8, padding: '16px 18px' }}>
              <p style={{ fontSize: 9, color: '#40b068', fontFamily: 'Inter, sans-serif', letterSpacing: '.12em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 10 }}>Established Facts</p>
          ...
          {extraction.disputed_areas?.length > 0 && (
            <div style={{ background: '#180808', border: '1px solid #401818', borderRadius: 8, padding: '16px 18px' }}>
              <p style={{ fontSize: 9, color: '#c05050', fontFamily: 'Inter, sans-serif', letterSpacing: '.12em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 10 }}>Disputed Areas</p>
              <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
          {extraction.legal_issues?.length > 0 && (
            <div style={{ background: '#0e0818', border: '1px solid #281840', borderRadius: 8, padding: '16px 18px' }}>
              <p style={{ fontSize: 9, color: '#8060c0', fontFamily: 'Inter, sans-serif', letterSpacing: '.12em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 10 }}>Legal Issues Identified</p>
              <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
                {extraction.legal_issues.map((l, i) => (
                  <li key={i} style={{ fontSize: 13, color: T.sub, fontFamily: "'Cormorant Garamond', serif", lineHeight: 1.7, marginBottom: 5, paddingLeft: 14, position: 'relative' }}>
                    <span style={{ position: 'absolute', left: 0, color: '#8060c0', fontSize: 8, top: 4 }}>●</span>{l}
                  </li>
                ))}
              </ul>
            </div>
          ...
          {extraction.gaps_identified?.length > 0 && (
            <div style={{ background: '#1a1000', border: '1px solid #3a2800', borderRadius: 8, padding: '16px 18px' }}>
              <p style={{ fontSize: 9, color: '#c08030', fontFamily: 'Inter, sans-serif', letterSpacing: '.12em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 10 }}>Gaps Identified</p>
              <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
        ...
        {extraction.initial_risks?.length > 0 && (
          <div style={{ background: '#0d0d18', border: '1px solid #181828', borderRadius: 8, padding: '16px 20px', marginBottom: 14 }}>
            <p style={{ fontSize: 9, color: '#c05050', fontFamily: 'Inter, sans-serif', letterSpacing: '.12em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 12 }}>Initial Risk Flags</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {extraction.initial_risks.map((r, i) => {
                const rc = RISK_SEV_C[r.severity] || RISK_SEV_C.MEDIUM;
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                    <span style={{ background: rc.bg, border: `1px solid ${rc.bdr}`, color: rc.col, fontSize: 8, padding: '2px 6px', borderRadius: 2, fontFamily: 'Inter, sans-serif', letterSpacing: '.06em', fontWeight: 600, flexShrink: 0, marginTop: 2 }}>
                      {r.severity}
                    </span>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => goBack(1)} style={{ background: 'transparent', border: '1px solid #1e1e2e', color: T.mute, borderRadius: 5, padding: '12px 20px', fontSize: 13, fontFamily: 'Inter, sans-serif', cursor: 'pointer', letterSpacing: '.04em' }}>
            ← Edit Facts
          </button>
          <button
            onClick={generateFollowUp}
            disabled={loading}
            style={{ flex: 1, background: loading ? '#101018' : 'linear-gradient(135deg,#c4a030,#a07820)', color: loading ? '#2a2a38' : '#05050c', border: 'none', borderRadius: 6, padding: '13px', fontSize: 17, fontFamily: "'Cormorant Garamond', serif", cursor: loading ? 'not-allowed' : 'pointer', fontWeight: 600, letterSpacing: '.04em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
    ...
    return (
      <div style={{ animation: 'fadeUp .3s ease' }}>
        <div style={{ marginBottom: 20 }}>
          ...
          <h2 style={{ fontSize: 22, color: T.goldL, fontFamily: "'Cormorant Garamond', serif", fontWeight: 300, marginBottom: 6 }}>Targeted Questions</h2>
          <p style={{ fontSize: 13, color: T.dim, fontFamily: 'Inter, sans-serif', lineHeight: 1.6 }}>
        ...
        {followUpQs.map((q, i) => (
          <div key={q.id} style={{ background: '#0d0d18', border: '1px solid #181828', borderRadius: 8, padding: '18px 20px', marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 10 }}>
              <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#1a1500', border: `1px solid ${ACC}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 2 }}>
                <span style={{ fontSize: 10, color: ACC, fontFamily: 'Inter, sans-serif', fontWeight: 700 }}>{i + 1}</span>
              </div>
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: 15, color: T.text, fontFamily: "'Cormorant Garamond', serif", lineHeight: 1.65, marginBottom: q.purpose ? 4 : 0 }}>{q.question}</p>
                {q.purpose && (
                  <p style={{ fontSize: 11, color: T.mute, fontFamily: 'Inter, sans-serif', lineHeight: 1.5, fontStyle: 'italic' }}>{q.purpose}</p>
                )}
              </div>
              {followUpAs[q.id]?.trim() && (
                <span style={{ fontSize: 8, color: '#40b068', fontFamily: 'Inter, sans-serif', letterSpacing: '.06em', background: '#071810', border: '1px solid #1a4028', padding: '2px 6px', borderRadius: 2, flexShrink: 0, marginTop: 3 }}>✓</span>
              )}
            </div>
            <textarea
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 10, color: T.mute, fontFamily: 'Inter, sans-serif' }}>{answeredCount} of {followUpQs.length} answered</span>
          {canProceed && (
            <span style={{ fontSize: 9, color: '#40b068', fontFamily: 'Inter, sans-serif', letterSpacing: '.06em', background: '#071810', border: '1px solid #1a4028', padding: '2px 8px', borderRadius: 2 }}>
              Ready to proceed
            </span>
          )}
        </div>
        ...
        <ErrorBlock message={error} />
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => goBack(2)} style={{ background: 'transparent', border: '1px solid #1e1e2e', color: T.mute, borderRadius: 5, padding: '12px 20px', fontSize: 13, fontFamily: 'Inter, sans-serif', cursor: 'pointer' }}>
            ← Back
          </button>
          <button
            onClick={buildEvidenceMatrix}
            disabled={!canProceed}
            style={{ flex: 1, background: canProceed ? 'linear-gradient(135deg,#c4a030,#a07820)' : '#101018', color: canProceed ? '#05050c' : '#2a2a38', border: 'none', borderRadius: 6, padding: '13px', fontSize: 17, fontFamily: "'Cormorant Garamond', serif", cursor: canProceed ? 'pointer' : 'not-allowed', fontWeight: 600, letterSpacing: '.04em' }}>
            Build Evidence Map →
          </button>
        </div>
      </div>
    ...
    return (
      <div style={{ animation: 'fadeUp .3s ease' }}>
        <div style={{ marginBottom: 18 }}>
          ...
          <h2 style={{ fontSize: 22, color: T.goldL, fontFamily: "'Cormorant Garamond', serif", fontWeight: 300, marginBottom: 4 }}>Evidence Requirements</h2>
          <p style={{ fontSize: 13, color: T.dim, fontFamily: 'Inter, sans-serif', lineHeight: 1.6 }}>
          ...
          return (
            <div key={i} style={{ background: '#0d0d18', border: '1px solid #181828', borderRadius: 8, padding: '18px 20px', marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 14 }}>
                <span style={{ background: pc.bg, border: `1px solid ${pc.bdr}`, color: pc.col, fontSize: 8, padding: '3px 7px', borderRadius: 2, fontFamily: 'Inter, sans-serif', letterSpacing: '.07em', fontWeight: 600, flexShrink: 0, marginTop: 2 }}>
                  {item.priority}
                </span>
                <p style={{ fontSize: 15, color: T.text, fontFamily: "'Cormorant Garamond', serif", fontWeight: 600, lineHeight: 1.45, flex: 1 }}>{item.issue}</p>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                <div>
                  <p style={{ fontSize: 8, color: T.mute, fontFamily: 'Inter, sans-serif', letterSpacing: '.1em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 8 }}>Required</p>
                  {(item.evidence_needed || []).map((e, j) => (
                    <p key={j} style={{ fontSize: 12, color: T.sub, fontFamily: "'Cormorant Garamond', serif", lineHeight: 1.65, marginBottom: 4, paddingLeft: 10, position: 'relative' }}>
                      <span style={{ position: 'absolute', left: 0, color: T.mute, fontSize: 8, top: 3 }}>·</span>{e}
                    </p>
                  ))}
                </div>
                <div>
                  <p style={{ fontSize: 8, color: '#40b068', fontFamily: 'Inter, sans-serif', letterSpacing: '.1em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 8 }}>Available</p>
                  {(item.evidence_available || []).length > 0
                    ? (item.evidence_available || []).map((e, j) => (
                        <p key={j} style={{ fontSize: 12, color: '#60c088', fontFamily: "'Cormorant Garamond', serif", lineHeight: 1.65, marginBottom: 4, paddingLeft: 10, position: 'relative' }}>
                          <span style={{ position: 'absolute', left: 0, color: '#40b068', fontSize: 8, top: 3 }}>✓</span>{e}
                        </p>
                      ))
                    : <p style={{ fontSize: 11, color: '#2a2a3e', fontFamily: 'Inter, sans-serif', fontStyle: 'italic' }}>None identified</p>
                  }
                </div>
                <div>
                  <p style={{ fontSize: 8, color: '#c05050', fontFamily: 'Inter, sans-serif', letterSpacing: '.1em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 8 }}>Missing</p>
                  {(item.evidence_missing || []).length > 0
                    ? (item.evidence_missing || []).map((e, j) => (
                        <p key={j} style={{ fontSize: 12, color: '#d07070', fontFamily: "'Cormorant Garamond', serif", lineHeight: 1.65, marginBottom: 4, paddingLeft: 10, position: 'relative' }}>
                          <span style={{ position: 'absolute', left: 0, color: '#c05050', fontSize: 8, top: 3 }}>!</span>{e}
                        </p>
                      ))
                    : <p style={{ fontSize: 11, color: '#2a2a3e', fontFamily: 'Inter, sans-serif', fontStyle: 'italic' }}>None</p>
                  }
                </div>
              </div>
              {item.notes && (
                <p style={{ marginTop: 10, paddingTop: 9, borderTop: '1px solid #131320', fontSize: 12, color: T.mute, fontFamily: 'Inter, sans-serif', lineHeight: 1.6, fontStyle: 'italic' }}>{item.notes}</p>
              )}
            </div>
          );
        })}
        ...
        <ErrorBlock message={error} />
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => goBack(3)} style={{ background: 'transparent', border: '1px solid #1e1e2e', color: T.mute, borderRadius: 5, padding: '12px 20px', fontSize: 13, fontFamily: 'Inter, sans-serif', cursor: 'pointer' }}>
            ← Back
          </button>
          <button
            onClick={generatePackage}
            style={{ flex: 1, background: 'linear-gradient(135deg,#c4a030,#a07820)', color: '#05050c', border: 'none', borderRadius: 6, padding: '13px', fontSize: 17, fontFamily: "'Cormorant Garamond', serif", cursor: 'pointer', fontWeight: 600, letterSpacing: '.04em' }}>
  ...
  function Stage5() {
    if (loading) return (
      <div style={{ textAlign: 'center', padding: '68px 24px' }}>
        <div style={{ width: 38, height: 38, border: `3px solid ${T.bdr}`, borderTop: `3px solid ${ACC}`, borderRadius: '50%', margin: '0 auto 20px', animation: 'spin .9s linear infinite' }} />
        <p style={{ fontSize: 22, color: T.goldL, fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', marginBottom: 10 }}>Assembling Intelligence Package…</p>
        <p style={{ fontSize: 11, color: T.mute, fontFamily: 'Inter, sans-serif', letterSpacing: '.12em' }}>TRIAL INTELLIGENCE ENGINE · AFS ADVOCATES</p>
      </div>
    );
    return (
      <div style={{ animation: 'fadeUp .3s ease' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 20, flexWrap: 'wrap' }}>
          <div style={{ flex: 1 }}>
            ...
            <h2 style={{ fontSize: 22, color: T.goldL, fontFamily: "'Cormorant Garamond', serif", fontWeight: 300, marginBottom: 4 }}>Intelligence Package</h2>
          </div>
          <div style={{ display: 'flex', gap: 8, flexShrink: 0, flexWrap: 'wrap' }}>
            ...
            <button onClick={() => goBack(4)} style={{ background: 'transparent', border: '1px solid #1e1e2e', color: T.mute, borderRadius: 4, padding: '7px 14px', fontSize: 11, fontFamily: 'Inter, sans-serif', cursor: 'pointer' }}>
              ← Evidence Map
            </button>
            <button
              onClick={resetPipeline}
              style={{ background: 'transparent', border: '1px solid #3a1818', color: '#804040', borderRadius: 4, padding: '7px 14px', fontSize: 11, fontFamily: 'Inter, sans-serif', cursor: 'pointer', letterSpacing: '.04em' }}>
              ↺ Reset Pipeline
            </button>
          </div>
        </div>
        ...
        {intPkg && (
          <div style={{ background: '#0a0a14', border: `1px solid ${ACC}33`, borderRadius: 10, padding: '26px 28px', marginBottom: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18, paddingBottom: 14, borderBottom: '1px solid #131320', flexWrap: 'wrap' }}>
              <RoleBadge role={role} />
              <span style={{ fontSize: 12, color: T.mute, fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic' }}>{activeCase.caseName}</span>
              <span style={{ marginLeft: 'auto', fontSize: 10, color: '#2a2a3e', fontFamily: 'Inter, sans-serif' }}>
    ...
    <div style={{ animation: 'fadeUp .35s ease' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 22, flexWrap: 'wrap' }}>
        ...
        {(stage > 1 || rawFacts.trim()) && (
          <button
            onClick={resetPipeline}
            style={{ background: 'transparent', border: '1px solid #2a1818', color: '#604040', borderRadius: 4, padding: '6px 13px', fontSize: 10, fontFamily: 'Inter, sans-serif', cursor: 'pointer', letterSpacing: '.06em', flexShrink: 0 }}>
            ↺ Reset Pipeline
          </button>
        )}
      </div>
